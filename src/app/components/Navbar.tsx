        ...
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          {isArtisan && onTabChange ? (
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <Link to="/catalogo" className="flex items-center gap-2 px-3 py-2 rounded-xl text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-colors font-medium">
                <Store className="h-4 w-4" /> Productos
              </Link>
              {isAuthenticated && !isArtisan && (
                <Link to="/mis-pedidos" className="flex items-center gap-2 px-3 py-2 rounded-xl text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-colors font-medium">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            {isAuthenticated && (
              <div className="relative group" ref={bellRef}>
                {user?.role === 'artisan' ? (
                  <Link to="/perfil-artesano#notificaciones"
                ...
                {user?.role !== 'artisan' && bellOpen && (
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                            ...
                            <div key={order.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                              ...
                              <div className="flex-1">
                                <p className="text-sm font-semibold text-gray-800">
                                  {order.detalles?.[0]?.producto_nombre || 'Pedido'}
                                  {order.detalles?.length > 1 && ` + ${order.detalles.length - 1} más`}
                                </p>
                    ...
                    <div className="px-4 py-3 border-t border-gray-100 bg-gray-50">
                      <Link to="/mis-pedidos" onClick={() => setBellOpen(false)}
