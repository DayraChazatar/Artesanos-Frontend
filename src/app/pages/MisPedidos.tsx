      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5 space-y-5">
          <div className="bg-orange-50 border border-orange-100 rounded-xl px-4 py-3 text-sm">
            <p className="font-medium text-gray-800 mb-1">{order.items?.map(i => i.name).join(', ')}</p>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {isApproved && (
            <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-4">
              ...
              <p className="text-sm text-green-600">{rr?.adminResponse || 'El artesano aprobó tu solicitud.'}</p>
            </div>
          )}
...
export function MisPedidos() {
  ...
  // ── Filtros ─────────────────────────────────────────────────────────────────
  const orderDates = new Set(orders.map(o => o.date?.slice(0, 10)).filter(Boolean));
  ...
  const filteredOrders = orders
    ...
    .filter(o => !selectedDate || o.date?.slice(0, 10) === selectedDate)
    .sort((a, b) => {
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            <button onClick={fetchOrders} title="Actualizar"
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-4">
              ...
              {hasActiveFilters && (
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                  ...
                  <button onClick={() => { setSearch(''); setFilterStatus('Todos'); setSelectedDate(null); }}
              ...
              {filteredOrders.length === 0 ? (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-16 text-center px-4">
                  ...
                  {orders.length === 0
                    ...
                    : <button onClick={() => { setSearch(''); setFilterStatus('Todos'); setSelectedDate(null); }}
                        className="text-sm text-orange-600 hover:text-orange-700 font-medium">Ver todos los pedidos</button>
                  }
                </div>
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {['Estado','Pedido','Fecha','Productos','Total','Acciones'].map(h => (
                      <th key={h} className={`px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider ${h === 'Total' ? 'text-right' : 'text-left'}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {filteredOrders.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="text-center py-16 text-gray-400">
                        ...
                        <p className="text-xs mb-4">{emptySubtext}</p>
                        {orders.length === 0
                          ? <Link to="/catalogo"><Button className="bg-orange-600 hover:bg-orange-700">Explorar Catálogo</Button></Link>
                          : <button onClick={() => { setSearch(''); setFilterStatus('Todos'); setSelectedDate(null); }}
                              className="text-sm text-orange-600 hover:text-orange-700 font-medium">Ver todos los pedidos</button>
